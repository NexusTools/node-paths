...
const c = path.resolve(__dirname, "c");

const one = path.resolve(a, "one.json");
const two = path.resolve(c, "two.json");
const three = path.resolve(b, "three.json");
const z = path.resolve(b, "z.json");
...
describe('instance methods', function() {
    ...
    it('resolveAsync', function(cb){
        instance.resolveAsync("tests.js", function(err, resolved) {
            ...
            if(!resolved) return cb(new Error("tests.json could be resolved"));
            cb();
        });
    });
    it('resolveAsync lookDeep', function(cb){
        instance.resolveAsync("one.json", function(err, resolved) {
            if(err) return cb(err);
            if(!resolved) return cb(new Error("one.json could be resolved"));
            cb();
        }, null, true);
    });
    ...
    it('contaminate _paths', function(){
        instance._paths.push(new Date() as any);
        try {
